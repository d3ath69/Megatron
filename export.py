...
def export_pdf(data: dict, output_dir: str) -> str:
    ...
    filename = os.path.join(output_dir, f"megatron_SL{sl}_{safe}.pdf")
    ...
    story.append(Paragraph("MEGATRON", title_style))
    ...
    story.append(Paragraph(
        "Generated by MEGATRON — AI Penetration Testing Assistant | "
        "github.com/sooryathejas/MEGATRON | For authorized use only.",
        footer_style))
...
def export_html(data: dict, output_dir: str) -> str:
    ...
    os.makedirs(output_dir, exist_ok=True)
    safe = tgt.replace("https://","").replace("http://","").replace("/","_").replace(".","_")
    filename = os.path.join(output_dir, f"megatron_SL{sl}_{safe}.html")
    vuln_rows = ""
    for v in data["vulns"]:
...
<head>
<meta charset="UTF-8">
<title>Megatron Report — {tgt}</title>
<style>
...
<div class="footer">
  Generated by MEGATRON &mdash;
  <a href="https://github.com/sooryathejas/MEGATRON">github.com/sooryathejas/MEGATRON</a>
...
def export_menu(data: dict):
    ...
    output_dir = os.path.expanduser("~/MEGATRON/reports")
    os.makedirs(output_dir, exist_ok=True)
...
if __name__ == "__main__":
    print("\n\033[91m    MEGATRON — Standalone Report Exporter\033[0m")
